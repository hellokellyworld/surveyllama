...
const context_agent = async (query, topResultsNum, chromaCollection) => {
	...
	const results = await chromaCollection.query(undefined, Math.min(topResultsNum, count), undefined, query);
	...
	while (true) {
		if (taskList.length > 0) {
			...
			if (collectionLength > 0) {
				await chromaCollection.update(resultId, undefined, { task: task.taskName, result: result }, vector);
			} else {
				await chromaCollection.add(resultId, undefined, { task: task.taskName, result }, vector);
			}
